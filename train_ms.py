...
def run(rank, n_gpus, hps):
  ...
  logger.info(f"============> Data for training: {len(train_loader)}")
  for epoch in range(epoch_str, hps.train.epochs + 1):
    if rank==0:
      # train_and_evaluate(rank, epoch, hps, [net_g, net_d, w2v, aug], [optim_g, optim_d], [scheduler_g, scheduler_d], scaler, [train_loader, eval_loader], logger, [writer, writer_eval])
      train_and_evaluate(rank, epoch, hps, [net_g, net_d, wavlm_model, aug], [optim_g, optim_d], [scheduler_g, scheduler_d], scaler, [train_loader, eval_loader], logger, [writer, writer_eval])
    else:
      # train_and_evaluate(rank, epoch, hps, [net_g, net_d, w2v, aug], [optim_g, optim_d], [scheduler_g, scheduler_d], scaler, [train_loader, None], None, None)
      train_and_evaluate(rank, epoch, hps, [net_g, net_d, wavlm_model, aug], [optim_g, optim_d], [scheduler_g, scheduler_d], scaler, [train_loader, None], None, None)
    scheduler_g.step()
    scheduler_d.step()
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths) in enumerate(train_loader):
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      # y_hat, z_mask,\
      # (z, z_p, m_p, logs_p, m_q, logs_q) = net_g(w2v_x, spec_lengths, spec, spec_lengths, mel)
      y_hat, z_mask,\
